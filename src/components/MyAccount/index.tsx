...
//         <div className="max-w-[1200px] 2xl:max-w-[1400px] w-full mx-auto px-4 sm:px-8">
...
import Orders from "../Orders";
import BreadcrumbThree from "../Common/CartSidebarModal/BreadcrumbThree";
import { logout, changeFullName } from "@/services/authService";
import Wishlist from "../Wishlist";
import Address from "./Address";
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <section className="py-11">
        {/* max-width reduced 1200->1080, 1400->1260, px-8->px-7 */}
        <div className="max-w-[1080px] 2xl:max-w-[1260px] w-full mx-auto px-4 sm:px-7">
          {/* gap-12->gap-11, gap-24->gap-21 */}
          <div className="flex flex-col lg:flex-row gap-11 xl:gap-21">
            
            {/* LEFT SIDEBAR: lg:w-280 -> lg:w-252 */}
            <aside className="lg:w-[252px] flex-shrink-0 hidden lg:flex">
              {/* space-y-8 -> space-y-7 */}
              <nav className="space-y-7">
                <div>
                  {/* text-xl -> text-[18px], mb-4 -> mb-3.5 */}
                  <h3 className="text-[18px] font-bold text-[#000000] mb-3.5">My Account</h3>
                  <ul className="space-y-2.5">
                    <li>
                      <button
                        onClick={() => setActiveTab("account-details")}
                        className={`text-[12.6px] font-medium transition-colors ${activeTab === "account-details" ? "text-[#116DB2]" : "text-gray-400 hover:text-[#000000]"}`}
                      >
                        Personal
                      </button>
                    </li>
                    <li>
                      <button
                        onClick={() => setActiveTab("addresses")}
                        className={`text-[12.6px] font-medium transition-colors ${activeTab === "addresses" ? "text-[#116DB2]" : "text-gray-400 hover:text-[#000000]"}`}
                      >
                        Data Addresses
                      </button>
                    </li>
                    <li>
                      <button
                        onClick={() => setActiveTab("dashboard")}
                        className={`text-[12.6px] font-medium transition-colors ${activeTab === "dashboard" ? "text-[#116DB2]" : "text-gray-400 hover:text-[#000000]"}`}
                      >
                        Loyalty Dashboard
                      </button>
                    </li>
                ...
                <div>
                  <h3 className="text-[18px] font-bold text-[#000000] mb-3.5">Order Information</h3>
                  <ul className="space-y-2.5">
                    <li>
                      <button
                        onClick={() => setActiveTab("orders")}
                        className={`text-[12.6px] font-medium transition-colors ${activeTab === "orders" ? "text-[#116DB2]" : "text-gray-400 hover:text-[#000000]"}`}
                      >
                        Order History
                      </button>
                    </li>
                ...
                <div>
                  <h3 className="text-[18px] font-bold text-[#000000] mb-3.5">Wish List</h3>
                  <ul className="space-y-2.5">
                    <li>
                      <button
                        onClick={() => setActiveTab("wishlist")}
                        className={`text-[12.6px] font-medium transition-colors ${activeTab === "wishlist" ? "text-[#116DB2]" : "text-gray-400 hover:text-[#000000]"}`}
                      >
                        Modify Wish List
                      </button>
                    </li>
            ...
            {/* MAIN CONTENT AREA */}
            <main className="flex-1">
              {/* mb-10 -> mb-9, pb-6 -> pb-5.5 */}
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-9 gap-3.5 pb-5.5">
                {/* text-2xl -> text-[21.6px], lg:text-3xl -> lg:text-[27px] */}
                <h1 className="text-[21.6px] lg:text-[27px] font-bold text-[#000000] tracking-tight">
                  My Account | {fullName}
                </h1>
                <button
                  onClick={handleLogout}
                  // px-10 -> px-9, py-2.5 -> py-2, text-sm -> text-[12.6px]
              ...
              {/* DASHBOARD GRID: p-10 -> p-9, min-h-180 -> min-h-162 */}
              {activeTab === "dashboard" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 animate-fadeIn">
                  {dashboardCards.map((card, i) => (
                    <button
                      key={i}
                      onClick={() => setActiveTab(card.id)}
                      className="flex flex-col items-center justify-center text-center p-9 border border-gray-200 rounded-xl hover:border-[#116DB2] transition-all group min-h-[162px]"
                    >
                      {/* text-3xl -> text-[27px] */}
                      <span className="text-[27px] font-bold text-[#000000] mb-2.5 group-hover:text-[#116DB2]">
                        {card.title}
                      </span>
                      {/* text-md -> text-[14.4px] */}
                      <span className="text-[14.4px] font-bold text-gray-400 uppercase tracking-widest leading-relaxed">
                        {card.desc}
                      </span>
                    </button>
                  ))}
              ...
              {/* ACCOUNT DETAILS TAB: max-w-2xl -> max-w-[612px], space-y-6 -> space-y-5.5 */}
              {activeTab === "account-details" && (
                <div className="max-w-[612px] animate-fadeIn">
                  <form onSubmit={handleChangeName} className="space-y-5.5">
                    <div className="grid grid-cols-2 gap-3.5">
                      <div>
                        <label className="block text-[10.8px] font-bold mb-1.5 uppercase">First Name*</label>
                        <input
                          type="text"
                          value={firstName}
                          onChange={(e) => setFirstName(e.target.value)}
                          className="w-full border border-gray-200 p-2.5 rounded bg-[#FDFDFD] text-[13.5px] outline-none focus:border-[#116DB2]"
                          required
                        />
                      </div>
                      <div>
                        <label className="block text-[10.8px] font-bold mb-1.5 uppercase">Last Name*</label>
                        <input
                          type="text"
                          value={lastName}
                          onChange={(e) => setLastName(e.target.value)}
                    ...
                    {message.text && (
                      <p className={`text-[12.6px] font-bold ${message.type === "success" ? "text-green-600" : "text-red-600"}`}>
                        {message.text}
                      </p>
                    )}
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      // px-8 -> px-7, py-3 -> py-2.5
                      className={`bg-[#116DB2] text-white px-7 py-2.5 rounded-full font-bold text-[12.6px] uppercase transition-all ${isLoading ? "opacity-50 cursor-not-allowed" : "hover:bg-[#AD003A]"}`}
                    >
                      {isLoading ? "Saving..." : "Save Changes"}
                    </button>
                  </form>
