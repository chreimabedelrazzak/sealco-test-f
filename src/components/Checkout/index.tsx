...
const Checkout = () => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (response.existingBillingAddresses?.length > 0) {
          setBillingAddress({ ...emptyAddress, ...response.existingBillingAddresses[0] });
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsUpdatingTotals(true);
      const finalShipping = useDifferentShipping ? shippingAddress : billingAddress;
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                        <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#116DB2] text-white text-xs">1</span>
                        Billing Address
                      </h3>
                    ...
                    {billingProvided ? (
                      <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                        <div className="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm">
                          <div
                            onClick={() => setUseDifferentShipping(!useDifferentShipping)}
                            className="cursor-pointer flex items-center gap-2.5 font-bold text-sm uppercase text-gray-800 py-5 px-5.5 hover:bg-gray-50 transition-colors"
                          >
                            <input
                              type="checkbox"
                            ...
                            />
                            Ship to a different address?
                            <svg className={`fill-current ease-out duration-200 ml-auto ${useDifferentShipping && "rotate-180"}`} width="22" height="22" viewBox="0 0 22 22" fill="none">
                          ...
                          <div className={`p-6 border-t border-gray-100 bg-gray-50/30 ${useDifferentShipping ? "block" : "hidden"}`}>
                            <h3 className="text-md font-bold text-gray-800 mb-4 flex items-center gap-2">
                                <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#116DB2] text-white text-xs">2</span>
                      ...
                      <div className="p-6 border border-dashed border-gray-300 rounded-xl bg-gray-50 text-center">
                        <p className="text-sm text-gray-500 font-medium">Please complete the billing address above to enable shipping options.</p>
                      </div>
                    )}

                    {/* ORDER NOTES */}
                    <div className="pt-4">
                      <label className="block text-xs font-bold text-gray-800 mb-3 uppercase tracking-wide">Other Notes (optional)</label>
                      <textarea
                        value={orderNote}
                        onChange={(e) => setOrderNote(e.target.value)}
                        rows={3}
                  ...
                  <div className="sticky top-24 bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
                    ...
                    <button
                      ...
                      className={`w-full mt-10 text-white py-4 rounded-full font-bold uppercase text-sm tracking-wider transition-all ${
                        billingProvided && !isUpdatingTotals ? "bg-[#116DB2] hover:bg-black shadow-lg" : "bg-gray-300 cursor-not-allowed"
                      }`}
                    >
