...
const SingleItem = ({ item }) => {
  ...
  return (
    <><div className="flex flex-col md:flex-row items-start md:items-center py-6 border-b border-gray-100 md:border-none gap-5 md:gap-0 relative">
      
      {/* COLUMN 1: PRODUCT INFO */}
      <div className="w-full lg:min-w-[400px] flex items-center gap-4">
        <div className="flex-shrink-0  w-24 h-24 md:w-20 md:h-20 flex items-center justify-center">
          <Image 
            width={200} 
            height={200} 
            unoptimized 
        ...
        <div className="flex-1">
          <h3 className="text-dark font-bold text-sm md:text-base leading-snug hover:text-[#116DB2] transition-colors">
            <a href="#">{item.title}</a>
          </h3>
          {/* Mobile Only: Unit Price displayed under title */}
          <p className="text-gray-500 text-xs mt-1 md:hidden">Unit: ${item.discountedPrice}</p>
        </div>
      </div>
      ...
      <div className="w-full md:w-auto md:min-w-[200px] flex items-center justify-between md:justify-center">
        <span className="text-xs font-bold text-gray-400 uppercase md:hidden">Quantity</span>
        <div className="flex items-center bg-white border border-gray-3 shadow-sm overflow-hidden">
          <button
...
{/* 
    <div className="flex flex-col md:flex-row gap-y-5 md:items-center py-5 px-7.5">
      ...
      <div className="md:min-w-[50px] flex justify-end">
        <button
          onClick={() => handleRemoveFromCart()}
          aria-label="button for remove product from cart"
          className="flex items-center justify-center rounded-lg max-w-[38px] w-full h-9.5 bg-gray-2 border border-gray-3 text-dark ease-out duration-200 hover:bg-red-light-6 hover:border-red-light-4 hover:text-red"
