...
const CartSidebarModal = () => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-black/10" onClick={closeCartModal} />
      <div className="max-w-[1400px] mx-auto px-4 relative h-full">
        <div
          ref={modalRef}
          className={`absolute top-20 w-[calc(100%-32px)] max-w-[380px] bg-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-sm transition-all duration-300 transform 
            /* MOBILE: Centered */
            left-1/2 -translate-x-1/2 origin-top 
            /* DESKTOP: Aligned to right */
            md:left-auto md:right-4 md:translate-x-0 md:origin-top-right 
            ${isCartModalOpen ? "scale-100 opacity-100" : "scale-95 opacity-0"}
          `}
        >
          {/* Header */}
          <div className="px-5 py-5 flex items-center justify-between border-b border-[#EEEEEE]">
            <h2 className="font-bold text-[#000000] text-xl">My Bag</h2>
            <button onClick={closeCartModal} className="text-gray-400 hover:text-black transition-colors">
          ...
          {/* Item List */}
          <div className="max-h-[450px] overflow-y-auto ap-5 no-scrollbar">
          ...
          {cartItems.length > 0 && (
            <div className="p-5 bg-white border-t border-[#EEEEEE]">
              <div className="flex items-center justify-between mb-6">
                <p className="font-bold text-[#000000] text-base">Subtotal:</p>
                <div className="flex items-baseline gap-1 text-black font-bold">
                  <span className="text-xl text-[#000000]">
                    {Number(totalPrice).toFixed(0)}$
                  </span>
                  <span className="text-[12px] text-[#000000] uppercase">
                    TTC
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-3">
                    ...
                    onClick={closeCartModal}
                    href="/cart"
                    className="flex-1 flex justify-center items-center py-3 font-bold text-[#000000] hover:bg-[#116DB2] hover:border-[#116DB2] hover:text-white bg-white border border-[#EEEEEE] rounded-3xl hover:bg-gray-50 transition-colors text-sm"
                    >
                    View Bag
                    </Link>
                    <button
                    onClick={handleProceedToCheckout}
                    className="flex-1 flex justify-center items-center py-3 font-bold text-white bg-[#116DB2] rounded-3xl hover:bg-[#0d568d] transition-colors text-sm"
                    >
